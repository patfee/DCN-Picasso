...
# ---------- Settings ----------
PEDESTAL_HEIGHT_M = 6.0  # for hover context only
...
if height_df.empty or outreach_df.empty:
    ...
    f_angles, m_angles, height_itp, outre_itp, _ = build_interpolators(height_df, outreach_df, pd.DataFrame())
    ...
    # ---------- Outline helper (concave/convex auto) ----------
    def compute_auto_outline(points, concavity_scale=1.0):
        """Smooth concave outline following outermost points. Fallback to convex hull."""
    ...
    layout = html.Div(
        ...
        children=[
            ...
            html.P("Tabular data points with envelope and subdivision controls."),
            html.Div(
                className="two-col",
                children=[
                    html.Div(
    ...
    def _update_chart(main_factor, fold_factor, concavity_scale):
        ...
        # Dense sampling at chosen subdivision factors
        ...
        # Interpolated points (light blue diamonds)
        if dense_xy.size:
            hover_tmpl = (
            ...
            fig.add_trace(go.Scatter(
                ...
                customdata=pts[:, :2]  # (Folding, Main)
            ))

        # Original matrix points (solid)
        if orig_xy.size:
            fig.add_trace(go.Scatter(
                x=orig_xy[:, 0],
                y=orig_xy[:, 1],
        ...
        # Auto outline (concave preferred; convex fallback)
        outline = compute_auto_outline(orig_xy, concavity_scale=concavity_scale)
        ...
        # Layout styling to mirror the screenshot
        fig.update_layout(
        ...
        # Info line
        info = (f"Main-jib subdivision: {main_factor}× • "
                f"Folding-jib subdivision: {fold_factor}× • "
                f"Angle ranges — Main: {m_angles.min():.2f}°…{m_angles.max():.2f}°, "
